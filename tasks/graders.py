...
def grade_detect_recon(episode_log):
    ...
    if recon_step is None:
        return {"score": 0.5, "reason": "No attack"}
    health_score = max(0.0, min(1.0, (final_health - 0.5) / 0.5))
    if response_step is None:
        return {"score": _clamp(0.1 * health_score + 0.001), "reason": "No response"}
    delay = response_step - recon_step
    d_score = 1.0 if delay <= 2 else (0.7 if delay <= 5 else 0.3)
...
def grade_stop_exploit(episode_log):
    ...
    max_stage = 0
    total_cost = 0.0
    costs = {"do_nothing":0.0,"monitor_traffic":0.05,"block_ip":0.10,"scan_system":0.20,"throttle_bandwidth":0.10,"isolate_subsystem":0.30}
    ...
    if not attack_stopped:
        return {"score": _clamp(max(0.002, 0.15 - 0.03 * max_stage) * max(0.01, final_health)), "reason": f"Not stopped, max_stage={max_stage}"}
    stage_scores = {0: 0.5, 1: 0.95, 2: 0.80, 3: 0.5, 4: 0.2}
    ss = stage_scores.get(stop_stage, 0.2)
    hs = max(0.0, min(1.0, (final_health - 0.4) / 0.6))
    eff = min(0.95, max(0.0, 1.0 - total_cost) + 0.5)
    ...
    for e in episode_log:
        if e["info"].get("attack_stage", 0) >= 4:
            ...
            if e.get("action") in ("throttle_bandwidth","isolate_subsystem","scan_system"):
                lsc += 1
    if exfil:
        return {"score": 0.001, "reason": "Exfil completed"}
    hs = max(0.0, min(1.0, (final_health - 0.5) / 0.5))
    aq = (lsc / lst) if lst > 0 else 0.5
    sb = 0.15 if attack_stopped else 0.0
    return {"score": _clamp(hs * 0.45 + aq * 0.35 + sb - min(0.3, fp_rate * 0.5)), "reason": f"health={final_health:.2f}"}
